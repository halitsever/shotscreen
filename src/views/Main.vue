  ...
  >
    ...
    <div class="stage" ref="stageRef">
      ...
      <div class="stage-content" :style="{ transform: `scale(${scale})` }">
        <div
          class="frm"
          :data-style="prefs.frameStyle"
          :style="{ width: vp.w + 'px', height: vp.h + 'px' }"
        >
          <div class="frm-inner">
            <webview
              ref="webviewRef"
    ...
    <form class="top-pill" @submit.prevent="commitUrl" @click.stop>
      ...
      <input
        ...
        spellcheck="false"
        @blur="cancelUrlEdit"
        @keydown.escape.prevent="cancelUrlEdit"
      />
    ...
    <div class="corner corner-tr">
      <div class="hist-wrap">
        <button
          :class="['ico-btn', showHist && 'on']"
          @click.stop="toggleHist"
          title="Recent shots"
        >
    ...
    <div :class="['dock', !prefs.dockExpanded && 'compact']">
      ...
      <div ref="vpMenuRef" style="position: relative;">
        <button class="dock-b dock-b-pill" @click.stop="showVpMenu = !showVpMenu">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
            <template v-if="vp.icon === 'phone'"><rect x="7" y="3" width="10" height="18" rx="2"/><path d="M11 18h2"/></template>
            <template v-else-if="vp.icon === 'tablet'"><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M11 18h2"/></template>
            <template v-else-if="vp.icon === 'wide'"><rect x="2" y="6" width="20" height="12" rx="2"/></template>
            <template v-else-if="vp.icon === 'ruler'"><path d="M3 14l7-11 11 7-7 11z"/><path d="M7 9l1.5 1M10 7l2 1.5M13 5l1.5 1"/></template>
            <template v-else><rect x="3" y="4" width="18" height="12" rx="2"/><path d="M9 20h6M12 16v4"/></template>
          </svg>
        ...
        <div v-if="showVpMenu" class="pop" @click.stop>
          ...
          <button
            ...
            :key="v.id"
            :class="['pop-row', vpId === v.id && 'on']"
            @click="selectViewport(v.id)"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0;">
              <template v-if="v.icon === 'phone'"><rect x="7" y="3" width="10" height="18" rx="2"/><path d="M11 18h2"/></template>
              <template v-else-if="v.icon === 'tablet'"><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M11 18h2"/></template>
              <template v-else-if="v.icon === 'wide'"><rect x="2" y="6" width="20" height="12" rx="2"/></template>
              <template v-else><rect x="3" y="4" width="18" height="12" rx="2"/><path d="M9 20h6M12 16v4"/></template>
            </svg>
            <span class="pop-row-l">{{ v.label }}</span>
            <span class="pop-row-d">{{ v.w }} × {{ v.h }}</span>
            <svg v-if="vpId === v.id" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="dock-grp dock-seg">
        <button
          v-for="m in MODES"
          :key="m.id"
          :class="['dock-icon', mode === m.id && 'on']"
          :data-tooltip="m.tip"
          @click="selectMode(m.id)"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
            <template v-if="m.icon === 'monitor'"><rect x="3" y="4" width="18" height="12" rx="2"/><path d="M9 20h6M12 16v4"/></template>
            <template v-else-if="m.icon === 'fullpage'"><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M9 8h6M9 12h6M9 16h4"/></template>
            <template v-else><path d="M3 7l9-4 9 4-9 4z"/><path d="M3 12l9 4 9-4"/></template>
          </svg>
        </button>
      </div>
      ...
      <div class="dock-grp">
        ...
        <button
          :class="['dock-icon', opts.dark && 'on']"
          data-tooltip="Force dark mode on page"
          @click="toggleDark"
        >
        ...
        <button
          :class="['dock-icon', opts.hideCookies && 'on']"
          data-tooltip="Hide cookie banners"
          @click="toggleCookies"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
...
const VIEWPORTS = [
  ...
  { id: "iphone-pro", label: "iPhone 15 Pro Max",  w: 430,  h: 932,  icon: "phone"   },
...
const MODES = [
  { id: "viewport", label: "Viewport", icon: "monitor",  tip: "Viewport — visible area only"    },
  { id: "fullpage", label: "Full Page", icon: "fullpage", tip: "Full Page — entire scroll height" },
];
...
const url        = ref("https://halitsever.github.io/shotscreen-browser/");
...
const urlDraft   = ref("");
const opts       = reactive({ delay: 0, hideCookies: true, hideStickies: false, dark: false, noAnim: true, format: "PNG", dpr: 2 });
const shots      = ref([]);
...
const scale = computed(() => {
  const padX = 80, padY = 180;
  return Math.min(
    1,
    (stageSize.w - padX * 2) / vp.value.w,
    (stageSize.h - padY)     / vp.value.h,
  );
});
...
const commitUrl = () => {
  ...
  const normalized = urlDraft.value.startsWith("http")
    ? urlDraft.value
    : `https://${urlDraft.value}`;
  url.value = normalized;
  editingUrl.value = false;
};

const cancelUrlEdit = () => { editingUrl.value = false; };
const clearUrl = () => { url.value = "about:blank"; };

const goBack  = () => webviewRef.value?.goBack();
const reload  = () => webviewRef.value?.reload();

const toggleHist  = () => { showHist.value = !showHist.value; };
const clearShots  = () => { shots.value.splice(0); };
...
const COOKIE_SEL = '[class*="cookie" i],[id*="cookie" i],[class*="consent" i],[id*="consent" i],[class*="gdpr" i],[id*="gdpr" i],[aria-label*="cookie" i],[aria-label*="consent" i],.cc-window,#onetrust-consent-sdk';
...
const toggleDark = async () => {
  ...
  } catch (e) { console.error('Dark toggle failed:', e); }
};
...
const toggleCookies = async () => {
  ...
  } catch (e) { console.error('Cookie toggle failed:', e); }
};
const selectMode    = (id) => { mode.value = id; };
...
const handleCapture = async () => {
  ...
  flash.value = true;
  setTimeout(() => { flash.value = false; }, 380);
  ...
  try {
    ...
    if (mode.value === 'fullpage') {
      ...
      restoreFullPage = () => { wv.style.flex = origFlex; wv.style.height = origH; };
    }

    const raw = await window.electronAPI.captureWebview({ format: opts.format, dpr: opts.dpr });
...
.window[data-stage="void"] .stage-bg { background: #050506; }
.window[data-stage="mesh"] .stage-bg {
  background:
    radial-gradient(600px 400px at 25% 25%, #4a3370 0%, transparent 60%),
    radial-gradient(700px 500px at 75% 70%, #2a4d7a 0%, transparent 60%),
    radial-gradient(500px 300px at 50% 50%, #1a3a3a 0%, transparent 60%),
    #0a0a0e;
}
...
.corner-tr { top: 18px; right: 18px; display: flex; gap: 8px; }
...
.top-pill {
  ...
  min-width: 320px;
  max-width: 540px;
  box-shadow: 0 8px 24px rgba(0,0,0,0.35), 0 0 0 0.5px rgba(255,255,255,0.05) inset;
  -webkit-app-region: no-drag;
}
.top-pill .lock {
  width: 22px;
...
.top-pill .url:hover { background: rgba(255,255,255,0.06); }
.top-pill .host { color: white; }
.top-pill .path { color: rgba(255,255,255,0.5); }
.top-pill input {
  flex: 1;
  font-size: 12.5px;
  letter-spacing: -0.01em;
...
.top-pill .x:hover { background: rgba(255,255,255,0.16); color: white; }
...
.ico-btn:hover { background: rgba(40,40,44,0.85); color: white; }
.ico-btn.on    { background: rgba(255,255,255,0.16); color: white; border-color: rgba(255,255,255,0.2); }
.ico-badge {
  position: absolute;
  top: -3px;
  right: -3px;
...
.hist-pop {
  ...
  border: 0.5px solid rgba(255,255,255,0.1);
  border-radius: 12px;
  box-shadow: 0 24px 60px rgba(0,0,0,0.5), 0 0 0 0.5px rgba(255,255,255,0.05) inset;
  padding: 6px;
  z-index: 10;
  color: white;
  -webkit-app-region: no-drag;
...
.hist-h button { color: rgba(255,255,255,0.5); padding: 4px; border-radius: 4px; cursor: pointer; }
.hist-h button:hover { color: white; background: rgba(255,255,255,0.08); }
.hist-list { display: flex; flex-direction: column; gap: 1px; }
.hist-empty { padding: 12px 10px; font-size: 12px; color: rgba(255,255,255,0.3); text-align: center; }
.hist-row {
  display: flex;
  align-items: center;
  gap: 10px;
...
.hist-at { font-size: 11px; color: rgba(255,255,255,0.5); font-variant-numeric: tabular-nums; }
.hist-act {
  width: 22px;
  height: 22px;
  border-radius: 5px;
...
.hist-act:hover { background: rgba(255,255,255,0.1); color: white; }
...
.status b { color: white; font-weight: 600; }
.status-dot {
  width: 6px;
  height: 6px;
  border-radius: 50%;
...
.dock-icon:disabled { color: rgba(255,255,255,0.25); cursor: default; }
.dock-icon:hover:not(:disabled),
.dock-b:hover    { background: rgba(255,255,255,0.08); color: white; }
.dock-icon.on,
.dock-b.on       { background: rgba(255,255,255,0.16); color: white; box-shadow: 0 0 0 0.5px rgba(255,255,255,0.1) inset; }
.dock-seg .dock-icon          { height: 30px; width: 32px; border-radius: 7px; }
.dock-seg .dock-icon.on       { background: white; color: #1a1a1d; box-shadow: none; }
.dock-icon-num {
  position: absolute;
  bottom: 4px;
  right: 4px;
...
/* tooltip */
.dock [data-tooltip] { overflow: visible; }
...
.dock-b-pill .lbl { font-weight: 500; color: white; }
.dock-b-pill .dim { color: rgba(255,255,255,0.5); font-variant-numeric: tabular-nums; font-size: 11px; font-weight: 400; }
button.cap-btn {
  height: 36px;
...
button.cap-btn:hover  { filter: brightness(1.08); }
button.cap-btn:active { transform: translateY(0.5px); filter: brightness(0.95); }

.dock.compact .dock-b-pill .lbl,
.dock.compact .dock-b-pill .dim,
.dock.compact .cap-btn span { display: none; }
.dock.compact .dock-b-pill  { padding: 0 10px; }
.dock.compact .cap-btn { padding: 0 12px; }
...
.pop-row:hover                { background: var(--accent); }
.pop-row:hover .pop-row-d     { color: rgba(255,255,255,0.85); }
.pop-row.on .pop-row-l        { font-weight: 500; }
.pop-row-l { flex: 1; }
.pop-row-d { color: rgba(255,255,255,0.5); font-variant-numeric: tabular-nums; font-size: 12px; }
.pop-divider { height: 0.5px; background: rgba(255,255,255,0.1); margin: 5px 0; }
.pop-custom {
  display: flex;
  align-items: center;
  gap: 5px;
...
.pop-custom > span  { color: rgba(255,255,255,0.5); font-size: 11px; }
.pop-custom > button {
  margin-left: auto;
  height: 24px;
  padding: 0 10px;
